...
- `Type 3자리 + SubType 3자리 + Serial 4자리`는 10진수 위치를 설명하는 표기다.
- 현재 구현에서 Type/SubType은 `uint8` 기반 값으로 다루며, 최대값은 각각 `255`다.
- 따라서 Type/SubType은 3자리 구간을 사용하지만 `000~999` 전체를 허용하지 않는다.
- Serial은 `0~9999` 구간을 사용하되, 실제 valid ItemID에서는 `0`을 invalid로 취급한다.
...
- `FItemID(uint32)`는 입력 정수를 그대로 `Value`에 저장한다.
- CSV/저장 데이터/직접 정수 입력처럼 `Set(EItemType, uint8, uint16)`를 거치지 않는 값은 반드시 `Validate()` 결과를 확인한다.
- 가능하면 raw 정수보다 Type/SubType/Serial을 분리해 `Set()` 또는 `UItemIDHelper::MakeItemID()` 경로로 생성한다.
...
| `Description` | 아이템 설명 |
| `DevState` | 개발/출시 상태 |
| `DevComment` | 에디터 전용 개발 메모. `WITH_EDITORONLY_DATA`에서만 포함 |
...
1. Message Log clear
2. 임시 RegisteredTables / ItemIndex 생성
3. `UItemDeveloperSettings::_ItemRegistryDataAsset` 동기 로드
4. `_ItemTables`의 DataTable을 임시 목록에 등록
5. 각 DataTable Row를 임시 ItemID 인덱스에 빌드
6. duplicate ItemID / invalid ItemID / unsupported RowStruct를 Message Log에 보고
7. Register와 BuildIndex가 모두 성공하면 실제 Registry를 임시 결과로 교체
8. 실패하면 기존 성공 Registry를 유지하고 실패 Notification 표시

중요 정책:

- DataAsset에 등록되지 않은 테이블은 Registry 대상이 아니다.
- `RefreshItemTable()`도 임의 테이블을 직접 등록하지 않고 전체 refresh만 수행한다.
- duplicate ItemID나 invalid Row가 있으면 새 Registry 적용은 실패한다.
- 실패 시 실패한 임시 인덱스는 폐기하고, 이전에 성공한 Registry가 있으면 그대로 유지한다.
- 이전 성공 Registry가 없는 초기 상태에서 실패하면 active Registry는 비어 있다.
...
- `RefreshRegistry()`는 임시 Registry를 먼저 빌드하고, 성공할 때만 active Registry를 교체한다. 실패 시 partial Registry를 적용하지 않고 기존 성공 Registry를 유지한다.
- 초기 상태처럼 이전 성공 Registry가 없을 때 refresh가 실패하면 active Registry는 비어 있다.
- `FItemID`는 10진수 조합 방식이며, 표기는 `Type 3자리 + SubType 3자리 + Serial 4자리`지만 현재 허용 범위는 `Type <= 255`, `SubType <= 255`, `Serial <= 9999`다. 즉 Type/SubType은 3자리 구간을 쓰되 `000~999` 전체를 허용하지 않는다.
- raw 정수 값을 직접 넣는 경로도 주의해야 한다. `FItemID(uint32)`는 입력값을 그대로 보관하므로, CSV/저장 데이터/직접 정수 입력처럼 `Set(EItemType, uint8, uint16)`를 거치지 않는 값은 반드시 `Validate()` 결과를 확인해야 한다. 가능하면 Type/SubType/Serial을 분리해 `Set()` 또는 `UItemIDHelper::MakeItemID()` 경로로 생성한다.
