...
bool FVNChoiceShotVisibilityTest::RunTest(const FString& _parameters)
{
	...
	if (visible_options.Num() == 3)
	{
		...
		TestFalse(TEXT("OptionA option is disabled when trust is too low"), visible_options[0].IsEnabled);
		TestTrue(TEXT("OptionB option is enabled by empty conditions"), visible_options[1].IsEnabled);
...
bool FVNChoiceShotSelectionTest::RunTest(const FString& _parameters)
{
	...
	choice_shot->SetOptions({ first_option, disabled_option });
