...
bool FVNEventHubVisibleEventsTest::RunTest(const FString& _parameters)
{
	...
	const FName current_day(TEXT("EventDay"));
	const FName required_flag(TEXT("CanSeeAdvanced"));

	UVNEventSetAsset* event_set = NewObject<UVNEventSetAsset>();
	event_set->_Events.Add(MakeEvent(1, current_day, EVNDaySlot::AfterSchool, 10, EVNEventRunMode::Normal, TEXT("EventDay_Choice")));
	event_set->_Events.Add(MakeEvent(2, current_day, EVNDaySlot::Night, 50, EVNEventRunMode::Normal, TEXT("EventDay_Night")));
	event_set->_Events.Add(MakeEvent(3, current_day, EVNDaySlot::None, 30, EVNEventRunMode::Normal, TEXT("EventDay_Common")));
	event_set->_Events.Add(MakeEvent(4, TEXT("DayA"), EVNDaySlot::AfterSchool, 40, EVNEventRunMode::Normal, TEXT("DayA_Event")));
	event_set->_Events.Add(MakeEvent(5, current_day, EVNDaySlot::AfterSchool, 90, EVNEventRunMode::Once, TEXT("EventDay_Once")));
	event_set->_Events.Add(MakeEvent(6, current_day, EVNDaySlot::AfterSchool, 80, EVNEventRunMode::Normal, NAME_None));

	FVNEventDef gated_event = MakeEvent(7, current_day, EVNDaySlot::AfterSchool, 70, EVNEventRunMode::Normal, TEXT("EventDay_Gated"));
	gated_event.ShowCond.All.Add(MakeBoolCondition(required_flag, EVNCompareOp::Equal, true));
	event_set->_Events.Add(gated_event);
	...
	story_state.CurrentDay = current_day;
	...
	story_state.BoolMap.Add(required_flag, true);
...
bool FVNEventHubAutoSelectionTest::RunTest(const FString& _parameters)
{
	...
	const FName current_day(TEXT("EventDay"));
	const FName can_run_auto_key(TEXT("CanRunAuto"));

	UVNEventSetAsset* event_set = NewObject<UVNEventSetAsset>();
	FVNEventDef low_auto = MakeEvent(1, current_day, EVNDaySlot::Morning, 10, EVNEventRunMode::Auto, TEXT("Auto_Low"));
	FVNEventDef locked_auto = MakeEvent(2, current_day, EVNDaySlot::Morning, 100, EVNEventRunMode::Auto, TEXT("Auto_Locked"));
	locked_auto.StartCond.All.Add(MakeBoolCondition(can_run_auto_key, EVNCompareOp::Equal, true));
	FVNEventDef high_auto = MakeEvent(3, current_day, EVNDaySlot::Morning, 50, EVNEventRunMode::Auto, TEXT("Auto_High"));
	FVNEventDef normal_event = MakeEvent(4, current_day, EVNDaySlot::Morning, 200, EVNEventRunMode::Normal, TEXT("Normal_High"));

	event_set->_Events = { low_auto, locked_auto, high_auto, normal_event };
	...
	story_state.BoolMap.Add(can_run_auto_key, true);
...
bool FVNEventHubEventLifecycleTest::RunTest(const FString& _parameters)
{
	...
	const FName current_day(TEXT("DayA"));
	const FName ready_key(TEXT("CanStartEvent"));
	const FName started_key(TEXT("EventStarted"));
	const FName trust_key(TEXT("TrustValue"));
	const FName complete_flag(TEXT("IsEventComplete"));

	FVNEventDef event_def = MakeEvent(10, current_day, EVNDaySlot::AfterSchool, 30, EVNEventRunMode::Once, TEXT("Event_Start"));
	event_def.StartCond.All.Add(MakeBoolCondition(ready_key, EVNCompareOp::Equal, true));
	event_def.OnStart.Add(MakeBoolChange(started_key, EVNStateOp::Set, true));
	event_def.OnComplete.Add(MakeIntChange(trust_key, EVNStateOp::Add, 1));
	event_def.CompleteFlag = complete_flag;
	...
	TestFalse(TEXT("Failed BeginEvent does not apply OnStart"), story_state.BoolMap.Contains(started_key));

	story_state.BoolMap.Add(ready_key, true);
	TestTrue(TEXT("BeginEvent applies OnStart when StartCond passes"), event_hub->BeginEventInStoryState(event_def, story_state));
	TestTrue(TEXT("OnStart bool is set"), story_state.BoolMap.FindRef(started_key));
	...
	TestEqual(TEXT("OnComplete int change is applied"), story_state.IntMap.FindRef(trust_key), 1);
	TestTrue(TEXT("CompleteFlag is set"), story_state.BoolMap.FindRef(complete_flag));
