...
캐릭터/단서/이벤트/엔딩처럼 ItemType이 확정된 도메인은 VN wrapper를 사용한다. 스프라이트/배경/BGM/SFX처럼 아직 전용 ItemType이 없는 범용 리소스 참조는 `FItemID`를 그대로 사용한다.
...
스프라이트/배경/BGM/SFX는 아직 `ItemCore`의 전용 `EItemType`으로 확정하지 않았으므로, 현재 Row 메타데이터에서는 `FItemID`로 보관한다. 후속 작업에서 전용 ItemType을 추가하면 `FBackgroundID` 같은 전용 ID 타입으로 분리한다.
...
| `_Lines` | 출력할 대사 줄 목록 |
| `_OnEnter` | Shot 시작 시 적용할 상태 변경 |
| `_OnComplete` | 모든 줄이 끝난 뒤 적용할 상태 변경 |
...
| `_ResultKey` | 선택한 ChoiceID를 저장할 `NameMap` 키 |
| `_OnEnter` | 선택지 표시 전 적용할 상태 변경 |
| `_OnComplete` | 선택 완료 뒤 공통 적용할 상태 변경 |
...
| `UVNEventSetAsset::_Events` | 제작 데이터로 관리할 이벤트 정의 배열 |
